...
const TodoListItem = ({ item }) => {
  ...
  const clickLike = () => {
    if (item.dislikeUsers.includes(userid))
      dispatch(voteUp({ userid: userid, ideaid: item.id, isDisliked: true }));
    else if (item.likeUsers.includes(userid)) return;
    else
      dispatch(voteUp({ userid: userid, ideaid: item.id, isDisliked: false }));
  };
  const clickDelete = () => {
  ...
  const clickDislike = () => {
    if (item.likeUsers.includes(userid))
      dispatch(voteDown({ userid: userid, ideaid: item.id, isLiked: true }));
    else if (item.dislikeUsers.includes(userid)) return;
    else
      dispatch(voteDown({ userid: userid, ideaid: item.id, isLiked: false }));
  };
  const clickComment = () => {
          ...
          {item.filepaths &&
            item.filepaths.map((file, index) => {
              return (
                ...
                >
                  # {item.filenames && item.filenames[index]}
                </a>
              );
            })}
        </div>
